...
class CabinetSceneCfg(InteractiveSceneCfg):
    """Scene configuration for the open drawer task."""
    ...
    cabinet_frame = FrameTransformerCfg(
        ...
        target_frames=[
            FrameTransformerCfg.FrameCfg(
                ...
                offset=OffsetCfg(
                    pos=(0.305, 0.0, 0.01),
                    rot=(0.5, 0.5, -0.5, -0.5), #aligna with end-effector frame
                ),
            ),
        ],
    )
...
class OpenDrawerEnvCfg(ManagerBasedRLEnvCfg):
    ...
    scene: CabinetSceneCfg = CabinetSceneCfg(env_spacing=2.0)
    ...
    def __post_init__(self) -> None:
        ...
        cab = self.scene.cabinet.init_state
        rob = self.scene.robot.init_state
        dx = rob.pos[0] - cab.pos[0]
        dy = rob.pos[1] - cab.pos[1]
        yaw = math.atan2(dy, dx)
        self.scene.cabinet.init_state.rot = (math.cos(yaw / 2.0), 0.0, 0.0, math.sin(yaw / 2.0))
...
# @configclass
# class CabinetDigitalTwinEnvCfg(OpenDrawerEnvCfg, ManagerBasedRLDigitalTwinEnvCfg):
#     """Configuration for the cabinet digital twin environment."""
