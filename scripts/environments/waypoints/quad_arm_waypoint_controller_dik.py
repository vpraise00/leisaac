...
class QuadArmWaypointController:
    ...
    def __init__(
        ...
        max_gripper_delta: float | None = 0.02,
    ):
    ...
    def step(self) -> tuple[torch.Tensor, bool]:
        ...
        # Limit per-step joint motion to avoid excessive penetration impulses
        joint_targets_nord = self._limit_joint_step(joint_pos_nord, joint_targets_nord)
        joint_targets_ost = self._limit_joint_step(joint_pos_ost, joint_targets_ost)
        joint_targets_west = self._limit_joint_step(joint_pos_west, joint_targets_west)
        joint_targets_sud = self._limit_joint_step(joint_pos_sud, joint_targets_sud)

        # Limit gripper motion per step for smoother closing
        nord_gripper_target = torch.full_like(joint_pos_nord[:, 5], self.current_waypoint.nord_gripper)
        ost_gripper_target = torch.full_like(joint_pos_ost[:, 5], self.current_waypoint.ost_gripper)
        west_gripper_target = torch.full_like(joint_pos_west[:, 5], self.current_waypoint.west_gripper)
        sud_gripper_target = torch.full_like(joint_pos_sud[:, 5], self.current_waypoint.sud_gripper)

        nord_gripper_target = self._limit_gripper_step(joint_pos_nord[:, 5], nord_gripper_target)
        ost_gripper_target = self._limit_gripper_step(joint_pos_ost[:, 5], ost_gripper_target)
        west_gripper_target = self._limit_gripper_step(joint_pos_west[:, 5], west_gripper_target)
        sud_gripper_target = self._limit_gripper_step(joint_pos_sud[:, 5], sud_gripper_target)

        # Compose action
        action = self._compose_quad_arm_action(
            joint_targets_nord, nord_gripper_target,
            joint_targets_ost, ost_gripper_target,
            joint_targets_west, west_gripper_target,
            joint_targets_sud, sud_gripper_target
        )
        ...
        # Use average error instead of requiring all arms to be within tolerance
        # This makes convergence more achievable for quad-arm system
        avg_pos_err = (pos_err_nord + pos_err_ost + pos_err_west + pos_err_sud) / 4.0
        max_pos_err = torch.max(torch.stack([pos_err_nord, pos_err_ost, pos_err_west, pos_err_sud]))

        # Converge if average error is low AND no single arm is too far off
        # Relaxed: max error can be up to 3x tolerance for better convergence
        within_pos = (avg_pos_err <= self.position_tol) and (max_pos_err <= self.position_tol * 3.0)
        ...
        if self.controls_orientation:
            ...
            avg_quat_err = (quat_err_nord + quat_err_ost + quat_err_west + quat_err_sud) / 4.0
            max_quat_err = torch.max(torch.stack([quat_err_nord, quat_err_ost, quat_err_west, quat_err_sud]))

            # Relaxed: max error can be up to 3x tolerance for better convergence
            within_rot = (avg_quat_err <= self.orientation_tol) and (max_quat_err <= self.orientation_tol * 3.0)
        else:
            within_rot = True
        ...
        # Debug output for convergence
        if not converged and (within_pos or self.hold_counter > 0):
            print(f"[Convergence] Hold: {self.hold_counter}/{self.required_hold} | "
                  f"Pos err - Avg={avg_pos_err.item():.4f} Max={max_pos_err.item():.4f} "
                  f"(N={pos_err_nord.item():.4f} O={pos_err_ost.item():.4f} "
                  f"W={pos_err_west.item():.4f} S={pos_err_sud.item():.4f}) | Tol={self.position_tol}")
    ...
    def _compose_quad_arm_action(
        self,
        nord_joints: torch.Tensor,
        nord_gripper: torch.Tensor,
        ost_joints: torch.Tensor,
        ost_gripper: torch.Tensor,
        west_joints: torch.Tensor,
        west_gripper: torch.Tensor,
        sud_joints: torch.Tensor,
        sud_gripper: torch.Tensor,
    ) -> torch.Tensor:
    ...
    def _limit_joint_step(self, current: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """Clamp joint displacement per step to keep IK motions physically feasible."""
        if self.max_joint_delta is None or self.max_joint_delta <= 0.0:
            return target

        delta = target - current
        delta = torch.clamp(delta, min=-self.max_joint_delta, max=self.max_joint_delta)
        return current + delta

    def _limit_gripper_step(self, current: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """Clamp gripper displacement per step for smoother open/close motion."""
        if self.max_gripper_delta is None or self.max_gripper_delta <= 0.0:
            return target

        delta = target - current
        delta = torch.clamp(delta, min=-self.max_gripper_delta, max=self.max_gripper_delta)
        return current + delta
